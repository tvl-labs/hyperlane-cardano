...
export function deserializeMerkleTree(datumMerkleTree: UplcData): HeliosMerkleTree {
  assert(datumMerkleTree.list.length === 2);
  const datumBranches = datumMerkleTree.list[0];
  const datumCount = datumMerkleTree.list[1];

  assert(datumBranches.list.length === TREE_DEPTH, `Invalid MerkleTree Datum. Branches length ${datumBranches.list.length} and must be ${TREE_DEPTH}`);
  const branches = datumBranches.list.map((branch) => H256.from(Buffer.from(branch.bytes)))
