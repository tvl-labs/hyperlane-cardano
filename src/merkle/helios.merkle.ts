...
export class HeliosMerkleTree {
  ...
  updateBranches(
    ...
    assert(i < TREE_DEPTH);
    if (size % 2 === 1) {
      const newBranches = [...this.branches.slice(0, i), node, ...this.branches.slice(i + 1)]
      assert(newBranches.length === TREE_DEPTH);
      return newBranches;
    }
    return this.updateBranches(i + 1, size / 2, hashConcat(this.hasher, this.branches[i], node))
  }
