...
import { getOutboxUtxos, type OutboxUtxo } from '../offchain/indexer/getOutboxUtxos';
...
async function prepareMessage(outboxUtxo: OutboxUtxo, senderWallet: Wallet): Promise<Message> {
...
async function findUsdcUtxos(wallet: Wallet): Promise<UsdcUtxo[]> {
  ...
  return walletUtxos.flatMap((utxo) =>
    utxo.value.assets.getTokens(mintingPolicyHash)
...
async function sendCardanoToKhalaniUsdcMessage() {
  ...
  const usdcBalance = usdcUtxos.reduce((total, { amount }) => total + amount, 0n);
  console.log(`Current USDC balance of ${wallet.address.toBech32()}: ${usdcBalance.toString()}`);
  ...
  const messageUtxo = await createOutboundMessage(outboxUtxo.utxo, message, wallet);
