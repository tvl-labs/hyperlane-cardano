...
function getMailboxAddress() {
  ...
  const chainsConfig: any = JSON.parse(
    fs.readFileSync(cardanoConfigPath, "utf-8")
  );
...
export async function sendKhalaniToCardanoUsdcMintMessage() {
  ...
  const mailbox = new ethers.Contract(getMailboxAddress(), MAILBOX_ABI, wallet);
  ...
  const { recipientAddress, recipientAddressHash } =
    getRecipientAddressAndHash();
  ...
  console.log(
    "Dispatched USDC minting transaction",
    KHALANI_BLOCK_EXPLORER_URL + transactionHash
  );
}
